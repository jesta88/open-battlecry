#include "xcr.h"
#include "../../engine/core/string.inl"
...
wb_xcr* wb_xcr_load(const char* path)
{
    if (!initialized)
    {
        log_error("XCR system not initialized. Call wb_xcr_init()");
        return NULL;
    }
    ...
    for (int i = 0; i < xcr_header->resource_count; i++)
    {
        ...
        const char* extension = io_path_find_extension(resource_header->file_name);
        if (extension == NULL)
        {
            continue;
        }
