...
#include "../../third_party/wuffs/wuffs.h"
...
static bool check_wuffs_status(wuffs_base__status status)
{
    ...
    {
		wb_log_error("%s", wuffs_base__status__message(&status));
        return false;
    }
	wb_log_error("%s", wuffs_base__status__message(&status));
    return true;
...
{
 //   // Input buffer
 //   u32 png_bytes_size = 0;
	//wb_file png_file = wb_file_size(file_name, &png_bytes_size, false);

 //   // TODO: Use a stack allocator
 //   u8* png_bytes = malloc(png_bytes_size);
 //   if (!png_bytes)
 //   {
 //       wb_log_error("Failed to allocate memory of size: %i", png_bytes_size);
 //       return;
 //   }
 //   wb_file_read(png_file, png_bytes, png_bytes_size);
 ...
 //   u32 width = wuffs_base__pixel_config__width(&image_config.pixcfg);
 //   u32 height = wuffs_base__pixel_config__height(&image_config.pixcfg);
 //   image->width = width;
 //   image->height = height;

 //   // Surface
 //   wuffs_base__pixel_format wuffs_pixel_format = wuffs_base__pixel_config__pixel_format(&image_config.pixcfg);
 //   u32 pixel_format = SDL_PIXELFORMAT_BGRA32;

 //   u32 plane_count = wuffs_base__pixel_format__num_planes(&wuffs_pixel_format);
 ...
 //   u32 work_buffer_length = work_buffer_range.max_incl;

 //   u8* work_buffer_data = malloc(work_buffer_length);
 //   if (!work_buffer_data)
 //   {
 //       wb_log_error("Failed to allocate memory of size: %i", work_buffer_length);
 //       return;
 //   }
...
void image_free(image_t* image)
{
    if (!image)
    {
        wb_log_error("%s", "Trying to free a null image.");
        return;
    }

    if (!image->sdl_surface)
    {
		wb_log_error("%s", "Trying to free a null SDL surface.");
        return;
    }
