#include "window.h"
#include "../graphics/graphics_old.h"
#include "log.h"
#include <assert.h>
#include <stdlib.h>
...
#endif
{
    ...
    client.window.hinstance = hInstance;
...
static void create_window(void)
{
    ...
    client.window.hwnd = CreateWindowExA(
        0,
        k_window_class,
        client.window.title,
        window_style,
...
static void adjust_window()
{
    if (client.window.fullscreen)
    {
        ...
        SetWindowLongA(client.window.hwnd, GWL_STYLE, WS_SYSMENU | WS_POPUP | WS_CLIPCHILDREN | WS_CLIPSIBLINGS | WS_VISIBLE);
    ...
    else
    {
        ...
        SetWindowLongA(client.window.hwnd, GWL_STYLE, windowStyle);
