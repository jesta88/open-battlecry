...
static void createInstance()
{
    ...
    sys_log_info("Vulkan instance version: %d.%d.%d", VK_API_VERSION_MAJOR(vk_instance_version),
...
static void pickPhysicalDevice()
{
    ...
    sys_log_info("Physical device: %s", s_physical_device_properties2.properties.deviceName);
}
...
static void createDevice()
{
    ...
    {
        sys_log_error("GPU doesn't support Descriptor Indexing feature: runtimeDescriptorArray.");
        return;
    }
    if (descriptor_indexing_features.descriptorBindingPartiallyBound == VK_FALSE)
    {
        sys_log_error("GPU doesn't support Descriptor Indexing feature: descriptorBindingPartiallyBound.");
        return;
    }
    if (dynamic_rendering_features.dynamicRendering == VK_FALSE)
    {
        sys_log_error("GPU doesn't support Dynamic Rendering feature: dynamicRendering.");
        return;
    }
...
static void createSwapchain(u32 width, u32 height, bool vsync)
{
    ...
    sys_log_info("Vulkan swapchain created with source_size %dx%d and %d images.", extent.width, extent.height,
...
#pragma warning(suppress : 6385)
    ...
    sys_log_info("Loaded Vulkan pipeline cache of %u bytes", cache_size);
}
...
static void savePipelineCache(const char* fileName)
{
    ...
    if (cache_size > 0)
    {
        ...
        sys_log_info("Saving pipeline cache of source_size %d bytes", (u32)cache_size);
