...
static VkBuffer s_transform_buffer;
static VmaAllocation s_transform_buffer_allocation;
static float view_projection_matrix[12];
...
void wbInitGraphics(const WbGraphicsDesc* graphics_desc)
{
	...
	const VkBufferCreateInfo buffer_info = {
			...
			.size = sizeof(float) * 2 * DEFAULT_MAX_STORAGE_BUFFERS
	};
	const VmaAllocationCreateInfo allocation_info = {
			.usage = VMA_MEMORY_USAGE_CPU_TO_GPU,
	};
	VkResult result = vmaCreateBuffer(s_allocator, &buffer_info, &allocation_info, &s_transform_buffer, &s_transform_buffer_allocation, NULL);
	assert(result == VK_SUCCESS);

	float positions[] = { -0.75f, -0.75f, -0.75f, 0.75f, 0.75f, 0.75f, 0.75f, -0.75f, 0.f, 0.f };
	void* data;
	result = vmaMapMemory(s_allocator, s_transform_buffer_allocation, &data);
	assert(result == VK_SUCCESS);

	memcpy(data, positions, sizeof(positions));

	vmaUnmapMemory(s_allocator, s_transform_buffer_allocation);
}
...
void wbDraw(void)
{
	...
	vkCmdSetScissor(cmd, 0, 1, &(const VkRect2D){
			.extent = { s_width, s_height }
	});

	vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, s_sprite_pipeline);
	vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, s_sprite_pipeline_layout, 0, 2, (const VkDescriptorSet[]){ s_sampled_image_descriptor_set, s_storage_buffer_descriptor_set  }, 0,
			NULL);

	const VkDescriptorBufferInfo buffer_info = (VkDescriptorBufferInfo){
			.buffer = s_transform_buffer,
			.offset = 0,
			.range = VK_WHOLE_SIZE,
	};
	...
	view_projection_matrix[0] = 2.0f / (1280.0f - 0.0f);
	view_projection_matrix[1] = 0.0f;
	view_projection_matrix[2] = 0.0f;

	view_projection_matrix[3] = 0.0f;
	view_projection_matrix[4] = 2.0f / (720.0f - 0.0f);
	view_projection_matrix[5] = 0.0f;

	view_projection_matrix[6] = 0.0f;
	view_projection_matrix[7] = 0.0f;
	view_projection_matrix[8] = 1.0f / (0.0f - 1.0f);

	view_projection_matrix[9] = -(1280.0f + 0.0f) / (1280.0f - 0.0f);
	view_projection_matrix[10] = -(720.0f + 0.0f) / (720.0f - 0.0f);
	view_projection_matrix[11] = 0.0f / (0.0f - 1.0f);

	vkCmdPushConstants(cmd, s_sprite_pipeline_layout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(view_projection_matrix), view_projection_matrix);

	vkCmdDraw(cmd, 5 * 6, 1, 0, 0);
...
static void pickPhysicalDevice()
{
	...
	wbLogInfo("Physical device: %s", s_physical_device_properties2.properties.deviceName);
}

static void createSurface(void* hwnd, void* hinstance)
{
	VkResult result = vkCreateWin32SurfaceKHR(s_instance, &(const VkWin32SurfaceCreateInfoKHR){
...
static void createDevice()
{
	...
	{
		wbLogError("GPU doesn't support Descriptor Indexing feature: runtimeDescriptorArray.");
		return;
	}
	if (descriptor_indexing_features.descriptorBindingPartiallyBound == VK_FALSE)
	{
		wbLogError("GPU doesn't support Descriptor Indexing feature: descriptorBindingPartiallyBound.");
		return;
	}
	if (dynamic_rendering_features.dynamicRendering == VK_FALSE)
	{
		wbLogError("GPU doesn't support Dynamic Rendering feature: dynamicRendering.");
		return;
	}
...
static void createDescriptorSets()
{
	...
	result = vkCreateDescriptorPool(s_device, &(const VkDescriptorPoolCreateInfo){
			...
			.pPoolSizes = (const VkDescriptorPoolSize[]){
					...
					{ VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE, DEFAULT_MAX_SAMPLED_IMAGES },
			},
	...
	result = vkCreateDescriptorPool(s_device, &(const VkDescriptorPoolCreateInfo){
			...
			.maxSets = 1,
			.poolSizeCount = 1,
			.pPoolSizes = &(VkDescriptorPoolSize){ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, DEFAULT_MAX_STORAGE_BUFFERS },
	...
	binding_flags_info.pBindingFlags = (const VkDescriptorBindingFlags[]){ 0,
	...
	set_layout_create_info.pBindings = (const VkDescriptorSetLayoutBinding[]){
			{
					...
					.pImmutableSamplers = &s_point_clamp_sampler,
			},
			{
					.binding = 1,
	...
	binding_flags_info.pBindingFlags = &(const VkDescriptorBindingFlags) {VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT | VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT};
...
static void createShaders()
{
	...
	const VkPipelineLayoutCreateInfo pipeline_layout_info = {
			...
			.setLayoutCount = 2,
			.pSetLayouts = (const VkDescriptorSetLayout[]){ s_sampled_image_descriptor_set_layout, s_storage_buffer_descriptor_set_layout },
...
static void loadPipelineCache(const char* fileName)
{
	...
	else
	{
		...
		if (cache_size > 0)
		{
			...
			fread(cache_data, cache_size, 1, file);
	...
	wbLogInfo("Loaded Vulkan pipeline cache of %u bytes", cache_size);
}
...
static void savePipelineCache(const char* fileName)
{
	...
	if (cache_size > 0)
	{
		...
		wbLogInfo("Saving pipeline cache of size %d bytes", (u32)cache_size);
