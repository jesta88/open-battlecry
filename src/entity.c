...
#define UNIT_MOVE_SPEED  80.0f
#define SEPARATION_DIST  18.0f
#define HIT_CHANCE_BASE  50
#define HEALTH_BAR_W     30.0f
#define HEALTH_BAR_H     4.0f
...
uint8_t direction_from_delta(float dx, float dy)
{
    ...
    int dir = (int)(angle / (2.0f * PI) * 8.0f + 0.5f) % 8;
    return (uint8_t)dir;
}
...
void units_update(unit_array* arr, const game_map* map, float dt, const combat_sounds* sounds)
{
        ...
        switch (u->state)
        {
            ...
            case UNIT_STATE_FIGHTING:
            {
                ...
                if (u->attack_timer <= 0.0f)
                {
                    ...
                    int hit_chance = (combat_a + combat_b > 0) ? (100 * combat_a) / (combat_a + combat_b) : HIT_CHANCE_BASE;
                    int roll = rand() % 100 + 1;
    ...
    const float sep_radius = SEPARATION_DIST;
...
void units_draw(const unit_array* arr, uint32_t white_tex)
{
    ...
    for (uint32_t idx = 0; idx < arr->count; ++idx)
    {
        ...
        {
            float bar_w = HEALTH_BAR_W, bar_h = HEALTH_BAR_H;
