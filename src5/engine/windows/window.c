...
static const u8 g_keycodes[512] = {
    [0x00B] = WK_BUTTON_0,
    [0x002] = WK_BUTTON_1,
    [0x003] = WK_BUTTON_2,
    [0x004] = WK_BUTTON_3,
...
static u16 translate_key(WPARAM wparam, LPARAM lparam)
{
...
{
    u16 key = translate_key(wparam, lparam);
    u16 scancode = (lparam >> 16) & 0x1FF;
    bool pressed = !((lparam >> 31) & 0x1);
    u8 modifier = get_modifier();
...
static void window_resized(int width, int height)
{
    ...
    event.resize.width = (u16)width;
    event.resize.height = (u16)height;
...
static int update_dimensions(void)
{
    ...
    g_window.width = (u16)roundf(window_width);
    g_window.height = (u16)roundf(window_height);
    return false;
}
